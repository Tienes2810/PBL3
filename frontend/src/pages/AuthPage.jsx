...
const Notification = ({ message, type, onClose }) => { if (!message) return null; const isSuccess = type === 'success'; return ( <div className={`fixed top-6 left-1/2 transform -translate-x-1/2 z-[100] flex items-center gap-3 px-6 py-4 rounded-2xl shadow-2xl backdrop-blur-md animate-fade-in-down border ${isSuccess ? 'bg-green-500/90 border-green-400 text-white' : 'bg-red-500/90 border-red-400 text-white'}`}> <span className="text-2xl">{isSuccess ? '🎉' : '⚠️'}</span> <div className="flex flex-col"> <span className="font-black uppercase text-[10px] tracking-widest opacity-80">{isSuccess ? 'Thành công' : 'Lỗi'}</span> <span className="font-bold text-sm">{message}</span> </div> <button onClick={onClose} className="ml-4 p-1 hover:bg-white/20 rounded-full transition-colors">✕</button> </div> ); };
...
const AuthPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isLogin && password !== confirmPassword) {
      const msg = t?.err_pass_match || "Mật khẩu xác nhận không khớp!";
      setErrorMsg(msg);
      showToast(msg, "error");
      setIsLoading(false);
      return;
    ...
    try {
      if (isLogin) {
        ...
        if (error || !data || data.password !== password) {
          ...
          showToast(t?.err_pass_wrong || "Đăng nhập thất bại", "error");
        } else {
            ...
            <div className="absolute right-0 top-full mt-2 w-56 bg-white rounded-2xl shadow-2xl border border-gray-100 p-2 z-50 animate-fade-in-down">
              {Object.keys(LANG_CONFIG).map((key) => (
                <button key={key} onClick={() => changeLanguage(key)} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold ${language === key ? 'bg-black text-white' : 'hover:bg-gray-100 text-gray-700'}`}>
                  <span className="text-xl">{LANG_CONFIG[key].flag}</span> {LANG_CONFIG[key].label}
                </button>
              ))}
            </div>
          )}
      ...
      <div className="relative z-10 w-full max-w-[420px] flex flex-col items-center px-4">
        ...
        <div className={`w-full bg-white px-8 ${isLogin ? 'py-8' : 'py-5'} rounded-[2.5rem] shadow-2xl border border-gray-100 relative z-10 transition-all`}>
          ...
          <div className={`text-center ${isLogin ? 'mb-6' : 'mb-3'} mt-2`}>
            {/* TIÊU ĐỀ: HỌC KANJI MỖI NGÀY */}
            <h1 className="text-2xl font-black text-gray-900 mb-1 uppercase tracking-tight leading-tight">
                {t?.auth_title || "HỌC KANJI MỖI NGÀY"}
            </h1>
            <p className="text-gray-400 text-sm italic font-medium">
                {isLogin ? (t?.auth_sub_login || "Kiên trì mỗi ngày...") : (t?.auth_sub_register || "Tạo tài khoản...")}
            </p>
          </div>

          <div className={`flex ${isLogin ? 'mb-6' : 'mb-4'} border-b border-gray-100`}>
            <button onClick={() => { setIsLogin(true); setErrorMsg(''); }} className={`w-1/2 pb-2 text-sm font-bold transition-all uppercase ${isLogin ? 'border-b-2 border-black text-black' : 'text-gray-300'}`}>
                {t?.auth_tab_login || "ĐĂNG NHẬP"}
            </button>
            <button onClick={() => { setIsLogin(false); setErrorMsg(''); }} className={`w-1/2 pb-2 text-sm font-bold transition-all uppercase ${!isLogin ? 'border-b-2 border-black text-black' : 'text-gray-300'}`}>
                {t?.auth_tab_register || "ĐĂNG KÝ"}
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className={`space-y-${isLogin ? '4' : '3'}`}>
            {/* TÊN ĐĂNG NHẬP / EMAIL */}
            <div>
              <label className="block text-[11px] font-bold text-gray-400 uppercase mb-1 ml-1">
                  {t?.auth_label_user || "TÊN ĐĂNG NHẬP / EMAIL"}
              </label>
              {/* NỀN XANH NHẠT: bg-[#eff6ff] */}
            ...
            <div className="relative">
              <label className="block text-[11px] font-bold text-gray-400 uppercase mb-1 ml-1">
                  {t?.auth_label_pass || "MẬT KHẨU"}
              </label>
              <div className="relative">
                <input type={showPassword ? "text" : "password"} required value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-5 py-2.5 bg-[#eff6ff] border border-transparent rounded-2xl focus:ring-2 focus:ring-black outline-none transition-all font-medium pr-12 text-slate-800" />
              ...
              {!isLogin && password.length > 0 && (
                <div className="mt-2 px-1 animate-fade-in-down">
                  ...
                  {/* TEXT CHUẨN: Ít nhất 8 ký tự, Chữ số, Ký tự đặc biệt */}
                  <div className="flex flex-wrap gap-x-3 gap-y-0.5">
                    <span className={`text-[8.5px] flex items-center gap-1 ${password.length >= 8 ? 'text-green-600 font-bold' : 'text-gray-400'}`}>{password.length >= 8 ? '✓' : '○'} {t?.auth_rule_length || "Ít nhất 8 ký tự"}</span>
                    <span className={`text-[8.5px] flex items-center gap-1 ${/\d/.test(password) ? 'text-green-600 font-bold' : 'text-gray-400'}`}>{/\d/.test(password) ? '✓' : '○'} {t?.auth_rule_digit || "Chữ số"}</span>
                    <span className={`text-[8.5px] flex items-center gap-1 ${/[!@#$%^&*(),.?":{}|<>]/.test(password) ? 'text-green-600 font-bold' : 'text-gray-400'}`}>{/[!@#$%^&*(),.?":{}|<>]/.test(password) ? '✓' : '○'} {t?.auth_rule_special || "Ký tự đặc biệt"}</span>
                  </div>
                </div>
              )}
            </div>
            ...
            {!isLogin && (
              <div>
                <label className="block text-[11px] font-bold text-gray-400 uppercase mb-1 ml-1">
                    {t?.auth_label_confirm || "XÁC NHẬN MẬT KHẨU"}
                </label>
            ...
            <div className={`${isLogin ? 'mt-6' : 'mt-3'}`}>
              <div className="relative">
                ...
                <div className="relative flex justify-center text-[10px]"><span className="px-3 bg-white text-gray-400 font-bold uppercase tracking-widest">{t?.auth_or || "HOẶC TIẾP TỤC VỚI"}</span></div>
              </div>
